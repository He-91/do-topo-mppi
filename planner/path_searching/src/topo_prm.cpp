...
bool TopoPRM::searchTopoPaths(const Vector3d& start, const Vector3d& goal,
    ...
    if (sample_points.size() < 10) {
        ROS_ERROR("[TopoPRM] 采样失败: 仅%zu个点，无法构建PRM图", sample_points.size());
        return false;
    }
    ...
    if (graph_nodes_.size() < 3) {
        ROS_ERROR("[TopoPRM] 图构建失败: 仅%zu个节点，无法进行路径搜索", graph_nodes_.size());
        clearGraph();
        return false;
    }
    ...
    if (raw_paths.empty()) {
        ROS_ERROR("[TopoPRM] DFS搜索失败: 未找到可行路径");
        clearGraph();
        return false;
    }
...
void TopoPRM::buildVisibilityGraph(const Vector3d& start, const Vector3d& goal,
    ...
    // K value: reduced from 28 to 18 to lower edge count and DFS search complexity
    // 原因: 在高密度障碍物环境中，过高的K导致边数爆炸，增加DFS搜索空间
    int K = 18;
    int edge_count = 0;
